...
def evaluation(pred, gt_path, iou_thresh=0.5):
    pred = get_preds(pred)
    norm_score(pred)
    facebox_list, event_list, file_list, hard_gt_list, medium_gt_list, easy_gt_list = get_gt_boxes("./ground_truth")
    ...
    for i in pbar:
        ...
        pred_list = pred[event_name.strip()]
        gt_bbx_list = facebox_list[i]

        for j in range(len(img_list)):
            pred_info = pred_list[str(img_list[j]).strip()]
